...
export function SalesPage() {
  ...
  const due = paymentMethod === 'CREDIT' ? Math.max(0, total - paid) : Math.max(0, total - paid);
  ...
  function updateQuantity(productId: string, quantity: number) {
    if (quantity <= 0) setCart(cart.filter((line) => line.product.id !== productId));
    else setCart(cart.map((line) => line.product.id === productId ? { ...line, quantity } : line));
  }

  async function checkout() {
    setError('');
    try {
      const sale = await saleService.create({ items: cart.map((line) => ({ productId: line.product.id, quantity: line.quantity, unitPrice: line.unitPrice })), discount: discountValue, paymentMethod, amountPaid: paid });
      setLastSale(sale); setCart([]); setDiscount('0'); setAmountPaid('0'); await refresh();
  ...
  return (
    <main style={{ maxWidth: 900, margin: '0 auto', padding: 16, fontFamily: 'system-ui' }}>
      ...
      <section>
        ...
        {cart.map((line) => <div key={line.product.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8, marginBottom: 8 }}>
          <span>{line.product.name}<br />{formatMoney(line.unitPrice)} / {line.product.unit}</span>
          <input aria-label={`Quantité ${line.product.name}`} type="number" min="0.01" step="any" value={line.quantity} onChange={(e) => updateQuantity(line.product.id, Number(e.target.value))} />
      ...
      <section>
        ...
        <label>Paiement <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethod)}>{Object.entries(paymentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        ...
        <button disabled={!cart.length} onClick={() => void checkout()}>Valider la vente</button>
      </section>
      ...
      <section><h2>Dernières ventes</h2>{saleRepository.listRecent(10).then ? <p>Historique disponible dans le repository.</p> : null}</section>
    </main>
  );
}
