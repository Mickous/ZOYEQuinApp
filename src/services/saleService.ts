...
import type { CreditAccount, CreditPayment } from '../models/customer';
    ...
    async create(input: CreateSaleInput): Promise<Sale> {
      ...
      return database.transaction('rw', database.sales, database.products, database.stockMovements, database.customers, database.creditAccounts, database.creditPayments, async () => {
        ...
        for (const item of saleItems) {
          ...
          await database.stockMovements.add({ id: createId('stock'), productId: product.id, type: 'OUT', quantityDelta: -item.quantity, quantityBefore: product.stockQuantity, quantityAfter: updated.stockQuantity, reason: `Vente ${sale.receiptNumber}`, referenceId: sale.id, unitCost: product.purchasePrice, createdAt: now });
        }
        await database.sales.add(sale);
